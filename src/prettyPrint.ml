...
let print_ast (pack, dl) pretty = 
	...
	let print_func_sign fs = match fs with
		| Ast.FunctionSig (f_list, t_op) ->
			begin
				print_string "(";
				let rec print_func_sign_helper f_list = match f_list with
					| [] -> ()
					| h::[] -> print_func_arg h
					| h::t ->
					begin
	...
	let print_struct_field_decl sfd = match sfd with
		| Ast.StructFieldDecl (sf_list) -> 
			begin
				let rec print_struct_field_helper sf_list = match sf_list with
					| [] -> ()
	...
	let rec print_stmt stmt = 
		...
		| Ast.IfStmt (s, e, s1_list, s2_list) ->
			begin
				print_string "if ";
				match s with 
				| None -> ()
		...
		| Ast.ReturnStatement (e_op)-> match e_op with
		...
		| Ast.SwitchStatement (s_op,e,case_list)->
			begin
				...
				| Some e -> print_expr e
				print_string "{ \n";
				List.iter (fun sl -> match sl with
					| Ast.SwitchCase (e_list,st_list) -> 
						begin
							print_string "case: ";
							List.iter print_stmt st_list;
						end
